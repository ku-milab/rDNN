function [predicted_label, accuracy, w, model] = multiKernelSVM_L1_RV(kernel, cvgrid, trainData, trainLabels, testData, testLabels, weights)
    ...
    for g_count = 1:cvgrid.gamma_TIMES
        ...
        switch kernel
            case 'rbf'
                ...
                cv_acc(c_count, g_count) = train(trainLabels, [(1:length(trainLabels))',combinedTrainKernel], opt);
            case 'linear'
                opt = ['-q -s 5 -e 0.001 -w1 1.0 -c ', num2str(2^(c_val)), ' -v ', num2str(cvgrid.FOLD_NUM)];
                cv_acc(c_count, g_count) = train(trainLabels, [(1:length(trainLabels))', combinedTrainKernel], opt);
        end
    end % g_count
end % c_count
...
switch kernel
    case 'rbf'
        ...
        model = train(trainLabels, [(1:length(trainLabels))', combinedTrainKernel], opt);
    case 'linear' % "-t 4": using predefined kernel
        opt = ['-q -s 5 -e 0.001 -w1 1.0 -c ', num2str(2^(Opt_c))];
        model = train(trainLabels, [(1:length(trainLabels))', combinedTrainKernel], opt);
end

% SVM Classification
[predicted_label, accuracy, w] = predict(testLabels, [(1:length(testLabels))', combinedTestKernel], model);
end