function [predicted_label, accuracy, w, model] = multiKernelSVM(kernel, cvgrid, trainData, trainLabels, testData, testLabels, weights)

addpath( genpath( fullfile( '..', 'Matlab_Toolbox', 'libsvm-mat-2.91-1' ) ) );
...
if length(trainData) ~= length(testData)
    fprintf( 2, 'The number of modalities should be same between the trianData and the testData.\n' );
end
    ...
    for g_count = 1:cvgrid.gamma_TIMES
        ...
        switch kernel
            case 'rbf'
                ...
                cv_acc(c_count, g_count) = svmtrain_lib(trainLabels, [(1:length(trainLabels))',combinedTrainKernel], opt);
            case 'linear'
                opt = ['-q -t 4 -h 0 -e 0.001 -w1 1.0 -c ', num2str(2^(c_val)), ' -v ', num2str(cvgrid.FOLD_NUM)];
                cv_acc(c_count, g_count) = svmtrain_lib(double(trainLabels), [(1:length(double(trainLabels)))', double(combinedTrainKernel)], opt);
        end
    end % g_count
end % c_count
...
switch kernel
    case 'rbf'
        ...
        model = svmtrain_lib(double(trainLabels), [(1:length(trainLabels))', double(combinedTrainKernel)], opt);
...
% SVM Classification
[predicted_label, accuracy, w] = svmpredict_lib(testLabels, [(1:length(testLabels))', combinedTestKernel], model);
